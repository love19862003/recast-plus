...
void ConvexVolumeTool::handleClick(const float* /*s*/, const float* p, bool shift)
{
	...
	if (shift)
	{
		// Delete
		int nearestIndex = -1;
		auto& pool = ptr->m_volumeOffConn.m_volumes;
    pool.call([&nearestIndex, p](const Volume* pvol, size_t index){
      const Volume& vol = *pvol;
      if (pointInPoly(vol.nverts, vol.verts, p) &&
          p[1] >= vol.hmin && p[1] <= vol.hmax){
        nearestIndex = index;
      }
    });
		...
		if (nearestIndex != -1)
		{
      m_sample->deleteConvexVolume(nearestIndex);
		}
	}
	else
	{
		...
		if (m_npts && rcVdistSqr(p, &m_pts[(m_npts-1)*3]) < rcSqr(0.2f))
		{
			if (m_nhull > 2)
			{
				...
				if (m_polyOffset > 0.01f)
				{
					...
					if (noffset > 0)
            m_sample->addConvexVolume(offset, noffset, minh, maxh, (unsigned char)m_areaType);
				}
				else
				{
          m_sample->addConvexVolume(verts, m_nhull, minh, maxh, (unsigned char)m_areaType);
				}
			}
