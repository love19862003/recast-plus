...
// 	if (!geom) return;
	...
	if (shift)
	{
		...
		int nearestIndex = -1;
		const float* verts = m_sample->getOffMeshConnectionVerts();
		for (int i = 0; i < m_sample->getOffMeshConnectionCount()*2; ++i)
		{
		...
		{
      m_sample->deleteOffMeshConnection(nearestIndex);
		}
	}
	else
	{
		...
		else
		{
			...
			m_sample->addOffMeshConnection(m_hitPos, p, m_sample->getAgentRadius(), m_bidir ? 1 : 0, area, flags);
			m_hitPosSet = false;
		}
	}
...
void OffMeshConnectionTool::handleRender()
{
  ...
  m_sample->drawOffConPools(&dd, true);
}
